...
Dates are stored with the class `Data` while datatimes are stored with POSIXct:
...
```
The class `POSIXct` stores the number of seconds since 01/01/1970, so its underlying datatype is numeric (it's a simple signed number):
...
And that's all! I'll be very happy to hear your comments, feedback, corrections and requests. You can get in touch with me on:
